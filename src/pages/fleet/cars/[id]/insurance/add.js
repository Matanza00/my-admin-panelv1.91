...
import { ArrowLeft, Calendar, UploadCloud } from "lucide-react"; // Icons for better UX

export default function AddInsuranceRecord() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/fleet/insurance", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        console.error("❌ Error adding insurance record:", await response.json());
      }
    } catch (error) {
      ...
      <div className="flex min-h-screen w-full justify-center items-center bg-gray-100">
        <div className="p-8 bg-white shadow-lg rounded-xl w-full max-w-md">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-3xl font-semibold text-gray-700">Add Insurance Record</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? "Processing..." : "Add Insurance Record"}
            </button>
